...
class MLP:
    ...
    def __init__(self, D, H, C, W1=None, b1=None, W2=None, b2=None, dtype=np.float32, seed=None) -> None: # noqa: PLR0913
        ...
        # Gradient buffers
        self.W1_grad = np.zeros_like(self.W1, dtype=dtype)
        self.b1_grad = np.zeros_like(self.b1, dtype=dtype)
        self.W2_grad = np.zeros_like(self.W2, dtype=dtype)
        self.b2_grad = np.zeros_like(self.b2, dtype=dtype)
    ...
    def backward(self, X: np.ndarray, y: np.ndarray, l2: float = 0.0) -> float:
        """Compute mean cross-entropy loss and gradients. No parameter updates.
        ...
        # Labels
        if y.ndim == 1: 
            y_onehot = np.eye(C, dtype=p.dtype)[y]
        ...
        logsumexp = np.log(expz.sum(axis=1, keepdims=True))            # (N,1)
        logp = z_shift - logsumexp                                     # (N,C)  == log softmax

        if y.ndim == 1:
            ce = -np.mean(logp[np.arange(N), y])
        else:
            ce = -np.mean((y_onehot * logp).sum(axis=1))

        # Gradients
        # NOTE: use slice assignment to avoid creating new arrays
        dz = (p - y_onehot) / N  # (N, C)
        self.W2_grad[:] = h.T @ dz  # (H, C)
        self.b2_grad[:] = dz.sum(axis=0)  # (C,)
        dh = dz @ self.W2.T  # (N, H)
        da1 = dh * (a1 > 0)
        self.W1_grad[:] = X.T @ da1  # (D, H)
        self.b1_grad[:] = da1.sum(axis=0)  # (H,)

        if l2 > 0.0:
            self.W2_grad += l2 * self.W2
            self.W1_grad += l2 * self.W1

        loss = float(ce + 0.5 * l2 * (np.sum(self.W1**2) + np.sum(self.W2**2)))  # L2 regularization term on weights
        return loss


    def parameters(self) -> List[Dict[str, np.ndarray]]:
        """Expose parameters and their gradients."""
        return [
            {"value": self.W1, "grad": self.W1_grad},
            {"value": self.b1, "grad": self.b1_grad},
            {"value": self.W2, "grad": self.W2_grad},
            {"value": self.b2, "grad": self.b2_grad}
        ]
    ...
    def sgd_step(self, X:np.ndarray, y:np.ndarray, lr:float = 1e-2, l2:float = 0.0) -> float: 
        """Perform a single SGD step on a batch (X,y). Helper function for optimizers.
